...
def sitebackup(self, data):
    ...
    if data['currsitetype'] in ['html', 'php', 'php70', 'php72', 'php74',
...
def site_package_check(self, stype):
    ...
    if stype in ['html', 'proxy', 'php', 'php70', 'php72', 'mysql', 'wp', 'wpsubdir',
        ...
        php_versions = ['php70', 'php72', 'php73', 'php74', 'php80', 'php81', 'php82', 'php83']
    ...
    if ((not pargs.php70) and (not pargs.php72) and (not pargs.php73) and (not pargs.php74) and
        (not pargs.php80) and (not pargs.php81) and (not pargs.php82) and (not pargs.php83) and
...
def detSitePar(opts):
    ...
    for key, val in opts.items():
        if val and key in ['html', 'php', 'mysql', 'wp',
                           'wpsubdir', 'wpsubdomain', 'php70', 'php72',
    ...
    if len(typelist) > 1 or len(cachelist) > 1:
        if len(cachelist) > 1:
        ...
        elif False not in [x in ('php70', 'mysql', 'html') for x in typelist]:
            sitetype = 'mysql'
            if not cachelist:
                cachetype = 'basic'
            else:
            ...
            if not cachelist:
                ...
                cachetype = cachelist[0]
        elif False not in [x in ('php70', 'mysql') for x in typelist]:
            sitetype = 'mysql'
            if not cachelist:
                cachetype = 'basic'
            else:
            ...
            if not cachelist:
                ...
                cachetype = cachelist[0]
        elif False not in [x in ('wp', 'php70') for x in typelist]:
            sitetype = 'wp'
            if not cachelist:
                cachetype = 'basic'
            else:
            ...
            if not cachelist:
             ...
             if not cachelist:
                 ...
                 cachetype = cachelist[0]
        elif False not in [x in ('wpsubdir', 'php70') for x in typelist]:
            sitetype = 'wpsubdir'
            if not cachelist:
                cachetype = 'basic'
            else:
            ...
            if not cachelist:
             ...
             if not cachelist:
                 ...
                 cachetype = cachelist[0]
        elif False not in [x in ('wpsubdomain', 'php70') for x in typelist]:
            sitetype = 'wpsubdomain'
            if not cachelist:
                cachetype = 'basic'
            else:
        ...
        if not typelist and not cachelist:
            sitetype = None
            cachetype = None
        elif (not typelist or "php70" in typelist) and cachelist:
            sitetype = 'wp'
            cachetype = cachelist[0]
