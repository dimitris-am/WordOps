...
class WOInfoController(CementBaseController):
    ...
    def info_php70(self):
        """Display PHP information"""
        version = os.popen("/usr/bin/php7.0 -v 2>/dev/null | "
                           "head -n1 | cut -d' ' -f2 |"
                           " cut -d'+' -f1 | tr -d '\n'").read
        config = configparser.ConfigParser()
        ...
        if os.path.exists('/etc/php/7.0/fpm/pool.d/www.conf'):
            config.read('/etc/php/7.0/fpm/pool.d/www.conf')
        else:
            Log.error(self, 'php-fpm pool config not found')
        if config.has_section('www'):
            wconfig = config['www']
        elif config.has_section('www-php70'):
            wconfig = config['www-php70']
        else:
            Log.error(self, 'Unable to parse configuration')
        www_listen = wconfig['listen']
        www_ping_path = wconfig['ping.path']
